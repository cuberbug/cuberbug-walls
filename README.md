...
>❗ Внимание! Инструменты переехали в [отдельный репозиторий](https://github.com/cuberbug/repo-tools). Для их подключения необходимо выполнить `git submodule update --init --recursive`. Позже процесс подключения и обновления сабмодуля будет автоматизирован.
