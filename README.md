...
[**Repo-Tools**](https://github.com/cuberbug/repo-tools) — это отдельный репозиторий с набором инструментов, подключающийся сюда посредством установки сабмодуля. Для его установки достаточно запустить `./tool-launcher` — он всё сделает сам, а содержимое репозитория **Repo-Tools** окажется в директории `./tools/`, после чего в новом окне терминала будет запущен скрипт `./tools/start`, который предложет автоматически установить виртуальное окружение Python в `./tools/.venv`, установит все необходимые зависимости и предложит запустить интерактивное TUI-меню, в котором доступен запуск любого из инструментов.
