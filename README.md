...
>Рекомендуется использовать `manager.sh` для запуска из файлового менеджера или `menu.sh` для запуска из терминала. В качестве альтернативы возможен и ручной запуск скриптов в основной логикой в случае, если вы не хотите устанавливать `gum` для работы интерактивного меню.
